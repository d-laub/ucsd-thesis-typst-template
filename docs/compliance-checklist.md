...
- [x] Non-primary authors not depersonalized as "et al." in the bibliography [P5] — **both** shipped CSLs (`styles/ieee-full-authors.csl`, `styles/ieee-full-authors-superscript.csl`) set `et-al-min="99" et-al-use-first="99"`; in each, the 8-author `octuple2023` entry prints all authors (last author "Davis" present, `grep -c "et al" == 0`)
- [x] Consistent citation/reference style throughout [P5] — a single numeric CSL drives both the in-text citations and the bibliography, whichever style is selected. Default `ieee-full-authors.csl`: `[1]`–`[3]` in body, `[n]`-numbered list. Optional `ieee-full-authors-superscript.csl`: superscript `¹`/`²,³`/`¹⁻³` in body with a matching `n.`-numbered list (the list is deliberately *not* bracketed there, so the two halves agree). Selecting one style per document is the author's responsibility; mixing them is out of scope for the template to enforce.
